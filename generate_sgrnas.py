...
def write_params_to_log(args):
    def join_or_drop(maybe_list):
        if maybe_list is None:
            ...
            raise TypeError(
                "join_or_drop() called on something other than None or list"
            )
...
def construct_perfect_sgrnas(args):
    ...
    for rec in SeqIO.parse(args.genbank, "gb"):
        upper_seq.append(str(rec.seq))
        lower_seq.append(revcomp(str(rec.seq)))
        if rec.features:
    ...
    genes = pd.DataFrame(
        genes, columns=["locus_tag", "gene_name", "strand", "start", "end"]
    )
    logging.info(f"\n{genes.head(10)}")
    logging.info(f"\n{genes.shape}")
    ...
    offtarget = list()
    for contig in range(len(upper_seq)):
        for m in re.finditer(r"(?=GG)", upper_seq[contig]):
            seed_sequence = upper_seq[contig][
                (m.start() - 1 - args.off_target_seed):(m.start() - 1)
            ]
            offtarget.append(seed_sequence)
        for m in re.finditer(r"(?=GG)", lower_seq[contig]):
            seed_sequence = lower_seq[contig][
                (m.start() - 1 - args.off_target_seed):(m.start() - 1)
            ]
            offtarget.append(seed_sequence)
                    ...
                    if gene_tag in args.locus_tag or "all" in args.locus_tag:
                        if "all" in args.locus_tag:
                            print(
                                ...
                                % (len(genes["start"].to_list()), gene_count, gene_tag),
                                end="\r",
                            )
                        else:
                            print(
                        ...
                        for m in re.finditer(
                            r"(?=CC)", str(nc_seq.upper())[0:(len(nc_seq) - 20 - 1)]
                        ):
...
def construct_mismatch_guides(args):
    ...
    for loc in range(20):
        for wt in range(len(DNA_BASES)):
            ...
            for mu in range(len(DNA_BASES)):
                n_mu = DNA_BASES[mu]
                if n_wt != n_mu:
                    P1 = float(weights[features.index(str(loc))]) + float(
                        weights[features.index(n_wt + n_mu)]
                    )
    ...
    for i in range(len(hitframe["n"].to_list())):
        ...
        count = 0
        for j in range(60):
            if sgRNA[math.floor(j / 3)] in DNA_BASES:
                index = (
                    12 * math.floor(j / 3)
                    + 3 * DNA_BASES.index(sgRNA[math.floor(j / 3)])
                    + j % 3
                )
                count = count + 1
    ...
    if args.downstream_remove is not None:
        ...
        for i in range(len(args.downstream_remove)):
            if len(args.downstream_remove[i]) != args.downstream:
                ...
                raise ValueError(message)
            if args.downstream_remove[i].upper().count("N") == len(
                args.downstream_remove[i]
            ):
                message = (
                    f"args.downstream_remove included {args.downstream_remove[i]} "
                    f"which filters all sgRNAs."
